<div class="uk-card">
	<div class="uk-card-header space-y-2">
		<h3 class="uk-h3">Team Members</h3>
		<p class="text-muted-foreground">Invite your team members to collaborate.</p>
	</div>

	<div class="uk-card-body space-y-6">
		{#each [{ name: 'Sofia Davis', email: 'm@example.com', role: 'Owner' }, { name: 'Jackson Lee', email: 'p@example.com', role: 'Member' }] as a}
			<div class="flex items-center justify-between">
				<div class="flex items-center space-x-4">
					<span class="relative flex h-10 w-10 shrink-0 overflow-hidden rounded-full bg-accent">
						<img
							alt=""
							class="aspect-square h-full w-full"
							src={`https://api.dicebear.com/8.x/lorelei/svg?seed=${a.name}`}
						/>
					</span>
					<div class="flex-1 space-y-2">
						<p class="font-medium leading-none">{a.name}</p>
						<p class="text-muted-foreground">{a.email}</p>
					</div>
				</div>
				<div class="h-10 w-52">
					<uk-select
						cls-custom="button: uk-input-fake w-full flex justify-between items-center; dropdown: min-w-52"
						icon="chevrons-up-down"
						drop="mode: click; pos: bottom-right"
						searchable
					>
						<select hidden>
							<option data-description="Can view and comment." value="viewer"> Viewer </option>
							<option data-description="Can view, comment and edit." value="developer">
								Developer
							</option>
							<option data-description="Can view, comment and manage billing." value="billing">
								Billing
							</option>
							<option data-description="Admin-level to all resources." value="owner">
								Owner
							</option>
						</select>
					</uk-select>
				</div>
			</div>
		{/each}
	</div>
</div>
