<script>
	...
	onMount(() => {
		// @ts-ignore
		dark = htmlElement.classList.contains('dark');
	});
</script>
			...
			<ul id="component-nav" class="uk-switcher max-w-2xl">
				<li class="uk-active space-y-6">
						...
						<div class="h-9">
							...
							>
								<select hidden>
									...
									<option>m@yahoo.com</option>
									<option>m@cloud.com</option>
								</select>
							</uk-select>
						</div>
						<div class="uk-form-help text-muted-foreground">
					...
					<div class="space-y-2">
						...
						/>
						<button class="uk-btn uk-btn-default" onclick={demo}> Add URL </button>
					</div>

					<div class="">
						<button class="uk-btn uk-btn-primary" onclick={demo}> Update profile </button>
					</div>
				</li>
				<li class="space-y-6">
					<div>
					...
					<div class="space-y-2">
						...
						<input class="uk-input w-60" id="date_of_birth" type="date" placeholder="Pick a date" />
						<div class="uk-form-help text-muted-foreground">
				...
				<li class="space-y-6">
					...
					<div class="space-y-2">
						...
						<div class="grid max-w-md grid-cols-2 gap-8">
							<a
								class={`block cursor-pointer items-center rounded-md border-2 border-muted p-1 ring-ring ${dark === false ? 'ring-2' : ''}`}
								onclick={() => {
									dark = false;
							...
							<a
								class={`block cursor-pointer items-center rounded-md border-2 border-muted bg-popover p-1 ring-ring ${dark === true ? 'ring-2' : ''}`}
								onclick={() => {
									dark = true;
					...
					<div class="">
						<button class="uk-btn uk-btn-primary" onclick={demo}> Update preferences </button>
					</div>
				</li>
				<li class="space-y-6">
					<div>
					...
					<div class="space-y-2">
						...
						{#each ['All new messages', 'Direct messages and mentions', 'Nothing'] as a, b}
							<label class="block text-sm" for={`notification_${b}`}>
								<input
									id={`notification_${b}`}
									class="uk-radio mr-2"
									name="notification"
					...
					<div>
						...
						<div class="space-y-4">
							{#each notifications as a, b}
								<div class="flex items-center justify-between rounded-lg border border-border p-4">
									<div class="space-y-0.5">
										<label class="text-base font-medium" for={`email_notification_${b}`}>
											{a.title}
										</label>
										<div class="uk-form-help text-muted-foreground">
											{a.description}
					...
					<div class="">
						<button class="uk-btn uk-btn-primary" onclick={demo}> Update notifications </button>
					</div>
				</li>
				<li class="space-y-6">
					<div>
					...
					<div class="space-y-2">
						...
						{#each ['Recents', 'Home', 'Applications', 'Desktop', 'Downloads', 'Documents'] as a, b}
							<label class="block text-sm" for={`display_${b}`}>
								<input class="uk-checkbox mr-2" type="checkbox" checked={[0, 1, 2].includes(b)} />
								{a}
							</label>
						{/each}
					</div>
					...
					<div class="">
						<button class="uk-btn uk-btn-primary" onclick={demo}> Update display </button>
					</div>
				</li>
			</ul>
		</div>
